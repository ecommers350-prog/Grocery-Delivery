      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <PackageIcon className="w-16 h-16 text-app-border mx-auto mb-4" />
              ...
              <Link key={order.id} to={`/orders/${order.id}`} className="block max-w-4xl bg-white rounded-2xl p-5 hover:shadow transition-all">
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <CalendarIcon className="w-3 h-3 text-app-text-light" />
                ...
                <div className="flex items-center gap-2 mb-3">
                  {order.items.slice(0, 4).map((item, i) => (
                    <img key={i} src={item.image} alt={item.name} className="w-12 sm:w-16 rounded-lg object-cover border border-app-border" />
                  ))}
                  {order.items.length > 4 && (
                    <div className="w-12 sm:w-16 rounded-lg bg-app-cream flex-center text-sm font-semibold text-app-text-light">+{order.items.length - 4}</div>
                  )}
                </div>
